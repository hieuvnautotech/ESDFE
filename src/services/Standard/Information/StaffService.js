...
const PrintStaff = async (params) => {
  try {
    return await axios.post(`${apiStaff}/print-staff`, params);
  } catch (error) {
    console.log(`ERROR: ${error}`);
  }
};
const getDepartment = async () => {
  try {
    return await axios.get(`${apiStaff}/get-department`);
  } catch (error) {
    console.log(`ERROR: ${error}`);
  }
};
export { getStaffList, createStaff, modifyStaff, deleteStaff, PrintStaff, getDepartment };
