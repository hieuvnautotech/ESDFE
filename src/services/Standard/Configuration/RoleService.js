...
const getAllTreeMenu = async (roleId) => {
  try {
    return await axios.get(`${apiName}/getMenuTree/${roleId}`);
  } catch (error) {
    console.log(`ERROR: ${error}`);
  }
};

const addMenuPermission = async (roleId, params) => {
  try {
    return await axios.post(`${apiName}/set-menu-permissions/${roleId}`, params);
  } catch (error) {
    console.log(`ERROR: ${error}`);
  }
};
