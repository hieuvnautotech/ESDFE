...
import FQCShippingDialog from './FQCShippingDialog';
import FQCShippingLot from './FQCShippingLot';
import FQCShippingAllLotDialog from './FQCShippingAllLotDialog';

export default function FQCShipping() {
  const intl = useIntl();
            ...
            <Grid item xs={4} style={{ textAlign: 'center' }}>
              {state.searchData.showDelete ? (
                <MuiIconButton
                  ...
                  text="delete"
                  disabled={params.row.OrderQty == params.row.ReceivedQty}
                />
              ) : (
                <MuiIconButton color="error" onClick={() => handleDelete(params.row)} text="undo" />
              )}
            </Grid>
            {params?.row?.isActived && (
              <>
                <Grid item xs={4} style={{ textAlign: 'center' }}>
                  <MuiIconButton color="warning" onClick={() => handleUpdate(params.row)} text="edit" />
                </Grid>
                <Grid item xs={4} style={{ textAlign: 'center' }}>
  ...
  const handleDelete = async (item) => {
    if (
      ...
      try {
        let res = await FQCShippingService.deleteFQCSO(item);
  ...
  async function fetchData() {
    ...
    const res = await FQCShippingService.getFQCSOList(params);
  ...
  return (
    <React.Fragment>
      ...
      <FQCShippingLot FQCSOId={FQCSOId} handleUpdateLot={handleUpdateLot} isActivedRow={isActivedRow} />
      ...
      <FQCShippingAllLotDialog RMModel={View} isOpen={isShowing2} onClose={toggle2} />
    </React.Fragment>
  );
}
