...
import { QCFrequencyService } from '@services';
import QCFrequencyDialog from './QCFrequencyDialog';
import { CREATE_ACTION, UPDATE_ACTION } from '@constants/ConfigConstants';

const TabFrequency = (props) => {
  const intl = useIntl();
  ...
  useEffect(() => {
    if (!_.isEmpty(updateData) && !_.isEqual(updateData, rowData) && isRendered) {
      ...
      const index = _.findIndex(newArr, function (o) {
        return o.QCFrequencyId == updateData.QCFrequencyId;
      });
      if (index !== -1) {
  ...
  async function fetchData() {
    ...
    const res = await QCFrequencyService.getQCFrequencyList(params);
  ...
  const handleDelete = async (row) => {
    ...
    if (window.confirm(message)) {
      try {
        let res = await QCFrequencyService.deleteQCFrequency({
          QCFrequencyId: row.QCFrequencyId,
  ...
  const getQCApply = async () => {
    const res = await QCFrequencyService.getQCApply();
    return res;
  };
  const columns = [
    { field: 'QCFrequencyId', headerName: '', flex: 0.3, hide: true },
    { field: 'Apply', hide: true },
    {
      ...
      flex: 0.01,
      filterable: false,
      renderCell: (index) => index.api.getRowIndex(index.row.QCFrequencyId) + 1 + (state.page - 1) * state.pageSize,
    },
    {
      field: 'action',
      headerName: '',
...

    {
      ...
      renderCell: (params) => {
        return (
          <Tooltip title={params.row.QCName ?? ''} className="col-text-elip">
            <Typography sx={{ fontSize: 14 }}>{params.row.QCName}</Typography>
          </Tooltip>
        );
      },
    },
    { field: 'QCApplyName', headerName: intl.formatMessage({ id: 'standardQC.QCApply' }), flex: 0.3 },
  ...
  return (
    <React.Fragment>
      ...
      <MuiDataGrid
        ...
        getRowId={(rows) => rows.QCFrequencyId}
