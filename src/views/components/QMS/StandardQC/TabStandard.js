...
import { QCStandardService } from '@services';
import QCStandardDialog from './QCStandardDialog';
import { CREATE_ACTION, UPDATE_ACTION } from '@constants/ConfigConstants';

const TabStandard = (props) => {
  const intl = useIntl();
  ...
  useEffect(() => {
    if (!_.isEmpty(updateData) && !_.isEqual(updateData, rowData) && isRendered) {
      ...
      const index = _.findIndex(newArr, function (o) {
        return o.QCStandardId == updateData.QCStandardId;
      });
      if (index !== -1) {
  ...
  async function fetchData() {
    ...
    const res = await QCStandardService.getQCStandardList(params);
  ...
  const handleDelete = async (row) => {
    ...
    if (window.confirm(message)) {
      try {
        let res = await QCStandardService.deleteQCStandard({
          QCStandardId: row.QCStandardId,
  ...
  const getQCApply = async () => {
    const res = await QCStandardService.getQCApply();
    return res;
  };
  const columns = [
    { field: 'QCStandardId', headerName: '', flex: 0.3, hide: true },
    ...
    { field: 'QCName', flex: 0.3, hide: true },
    {
      ...
      flex: 0.01,
      filterable: false,
      renderCell: (index) => index.api.getRowIndex(index.row.QCStandardId) + 1 + (state.page - 1) * state.pageSize,
    },
    {
      field: 'action',
      headerName: '',
  ...
  return (
    <React.Fragment>
      ...
      <MuiDataGrid
        ...
        getRowId={(rows) => rows.QCStandardId}
