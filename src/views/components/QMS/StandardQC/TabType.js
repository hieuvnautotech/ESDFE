...
import { QCTypeService } from '@services';
import QCTypeDialog from './QCTypeDialog';
...
const TabType = (props) => {
  ...
  useEffect(() => {
    if (!_.isEmpty(updateData) && !_.isEqual(updateData, rowData) && isRendered) {
      ...
      const index = _.findIndex(newArr, function (o) {
        return o.QCTypeId == updateData.QCTypeId;
      });
      if (index !== -1) {
  ...
  async function fetchData() {
    ...
    const res = await QCTypeService.getQCTypeList(params);
  ...
  const handleDelete = async (row) => {
    ...
    if (window.confirm(message)) {
      try {
        let res = await QCTypeService.deleteStandardQC({ QCTypeId: row.QCTypeId, row_version: row.row_version });
  ...
  const getQCApply = async () => {
    const res = await QCTypeService.getQCApply();
    return res;
  };
  const columns = [
    { field: 'QCTypeId', headerName: '', flex: 0.3, hide: true },
    { field: 'QCType', flex: 0.3, hide: true },
    { field: 'Apply', hide: true },
    {
      ...
      flex: 0.01,
      filterable: false,
      renderCell: (index) => index.api.getRowIndex(index.row.QCTypeId) + 1 + (state.page - 1) * state.pageSize,
    },
    {
      field: 'action',
      headerName: '',
