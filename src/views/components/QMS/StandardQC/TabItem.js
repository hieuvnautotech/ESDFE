...
import QCItemDialog from './QCItemDialog';
...
const TabItem = (props) => {
  ...
  useEffect(() => {
    if (!_.isEmpty(updateData) && !_.isEqual(updateData, rowData) && isRendered) {
      ...
      const index = _.findIndex(newArr, function (o) {
        return o.QCItemId == updateData.QCItemId;
      });
      if (index !== -1) {
  ...
  async function fetchData() {
    ...
    const res = await QCItemService.getQCItemList(params);
  ...
  const handleDelete = async (row) => {
    ...
    if (window.confirm(message)) {
      try {
        let res = await QCItemService.deleteQCItem({ QCItemId: row.QCItemId, row_version: row.row_version });
  ...
  const getQCApply = async () => {
    const res = await QCItemService.getQCApply();
    return res;
  };
  const columns = [
    { field: 'QCItemId', headerName: '', flex: 0.3, hide: true },
    ...
    { field: 'QCName', flex: 0.3, hide: true },
    {
      ...
      flex: 0.01,
      filterable: false,
      renderCell: (index) => index.api.getRowIndex(index.row.QCItemId) + 1 + (state.page - 1) * state.pageSize,
    },
    {
      field: 'action',
      headerName: '',
