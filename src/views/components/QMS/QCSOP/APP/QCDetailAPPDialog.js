...
const QCMasterAPPDetailDialog = ({ isOpen, onClose, QCAPPMasterId }) => {
  const intl = useIntl();
  ...
  const handleDelete = async (item) => {
    if (
      ...
      try {
        let res = await qcAPPService.deleteAPPDetail(item);
  ...
  async function fetchData() {
    const params = { page: 0, pageSize: 0, QCAPPMasterId: QCAPPMasterId };

    const res = await qcAPPService.getAPPDetail(params);
    if (res && isRendered) {
  ...
  const onSubmit = async (data) => {
    ...
    const res = await qcAPPService.createAPPDetail(data);
  ...
  const handleClear = async () => {
    if (window.confirm(intl.formatMessage({ id: 'general.confirm_delete' }))) {
      try {
        let res = await qcAPPService.clearAPPDetail(QCAPPMasterId);
        if (res && res.HttpResponseCode === 200) {
          SuccessAlert(intl.formatMessage({ id: 'general.success' }));
          await fetchData();
        } else {
          ...
          <Grid item xs={3} container spacing={2}>
            <Grid item xs={12}>
              <MuiAutocomplete
                ...
                fetchDataFunc={qcAPPService.getType}
            ...
            <Grid item xs={12}>
              <MuiAutocomplete
                ...
                fetchDataFunc={() => qcAPPService.getItem(values.QCTypeId)}
            ...
            <Grid item xs={12}>
              <MuiAutocomplete
                ...
                fetchDataFunc={() => qcAPPService.getStandard(values.QCTypeId, values.QCItemId)}
                    ...
                    return (
                      <tr key={index}>
                        ...
                        <td style={{ ...style.td, maxWidth: 100 }}>
                          <TextField fullWidth type="number" size="small" label="Result" />
                        </td>
...
export default QCMasterAPPDetailDialog;
