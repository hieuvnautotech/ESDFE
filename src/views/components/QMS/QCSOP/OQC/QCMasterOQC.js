...
import { qcOQCService, bomService } from '@services';
import { addDays, ErrorAlert, SuccessAlert, minusMonths } from '@utils';
import moment from 'moment';
import QCDetailOQCDialog from './QCDetailOQCDialog';
import QCMasterOQCDialog from './QCMasterOQCDialog';

const QCMasterOQC = (props) => {
  const intl = useIntl();
  const date = new Date();
  ...
  const [QCOQCMasterId, setQCOQCMasterId] = useState(null);
  ...
  const columns = [
    {
      ...
      align: 'center',
      filterable: false,
      renderCell: (index) => index.api.getRowIndex(index.row.QCOQCMasterId) + 1 + (state.page - 1) * state.pageSize,
    },
    { field: 'QCOQCMasterId', hide: true },
    ...
    {
      field: 'QCOQCMasterName',
      headerName: intl.formatMessage({ id: 'qcOQC.QCOQCMasterName' }),
      width: 250,
    },
    {
      field: 'OQCTypeName',
      headerName: intl.formatMessage({ id: 'qcOQC.OQCType' }),
      width: 150,
    },
    {
      field: 'ProductList',
  ...
  useEffect(() => {
    if (!_.isEmpty(updateData) && !_.isEqual(updateData, rowData)) {
      ...
      const index = _.findIndex(newArr, function (o) {
        return o.QCOQCMasterId == updateData.QCOQCMasterId;
      });
      if (index !== -1) {
  ...
  const handleConfirm = async (item) => {
    if (
      ...
      try {
        let res = await qcOQCService.confirmOQCMaster(item);
  ...
  const handleDelete = async (item) => {
    if (
      ...
      try {
        let res = await qcOQCService.deleteOQCMaster(item);
        if (res && res.HttpResponseCode === 200) {
          SuccessAlert(intl.formatMessage({ id: 'general.success' }));
          await fetchData();
        } else {
  ...
  const handleCopy = async (row) => {
    if (window.confirm(intl.formatMessage({ id: 'general.confirm_save' }))) {
      try {
        const res = await qcOQCService.copyOQCMaster(row);
  ...
  const handleDetail = async (row) => {
    setQCOQCMasterId(row.QCOQCMasterId);
    toggle2();
  };
  ...
  async function fetchData() {
    ...
    const params = {
      QCOQCMasterName: state.searchData.keyWord,
    ...
    const res = await qcOQCService.getOQCMasterList(params);
  ...
  return (
    <React.Fragment>
      ...
      <MuiDataGrid
        ...
        getRowId={(rows) => rows.QCOQCMasterId}
      ...
      <QCDetailOQCDialog isOpen={isShowing2} onClose={toggle2} QCOQCMasterId={QCOQCMasterId} />
    </React.Fragment>
  );
};
...
export default connect(mapStateToProps, mapDispatchToProps)(QCMasterOQC);
