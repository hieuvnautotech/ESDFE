...
import QCDetailMoldDialog from './QCDetailMoldDialog';
import QCMasterMoldDialog from './QCMasterMoldDialog';

const QCMasterMold = (props) => {
  const intl = useIntl();
  ...
  const [QCMoldMasterId, setQCMoldMasterId] = useState(null);
  ...
  const columns = [
    {
      ...
      align: 'center',
      filterable: false,
      renderCell: (index) => index.api.getRowIndex(index.row.QCMoldMasterId) + 1 + (state.page - 1) * state.pageSize,
    },
  ...
  useEffect(() => {
    if (!_.isEmpty(updateData) && !_.isEqual(updateData, rowData)) {
      ...
      const index = _.findIndex(newArr, function (o) {
        return o.QCMoldMasterId == updateData.QCMoldMasterId;
      });
      if (index !== -1) {
  ...
  const handleConfirm = async (item) => {
    if (
      ...
      try {
        let res = await qcMoldService.confirmMoldMaster(item);
  ...
  const handleDelete = async (item) => {
    if (
      ...
      try {
        let res = await qcMoldService.deleteMoldMaster(item);
        if (res && res.HttpResponseCode === 200) {
          SuccessAlert(intl.formatMessage({ id: 'general.success' }));
          await fetchData();
        } else {
  ...
  const handleCopy = async (row) => {
    if (window.confirm(intl.formatMessage({ id: 'general.confirm_save' }))) {
      try {
        const res = await qcMoldService.copyMoldMaster(row);
  ...
  const handleDetail = async (row) => {
    setQCMoldMasterId(row.QCMoldMasterId);
    toggle2();
  };
  ...
  async function fetchData() {
    ...
    const params = {
      QCMoldMasterName: state.searchData.keyWord,
      page: state.page,
      pageSize: state.pageSize,
      isActived: state.searchData.showDelete,
    };

    const res = await qcMoldService.getMoldMasterList(params);
  ...
  return (
    <React.Fragment>
      ...
      <MuiDataGrid
        ...
        getRowId={(rows) => rows.QCMoldMasterId}
      ...
      <QCDetailMoldDialog isOpen={isShowing2} onClose={toggle2} QCMoldMasterId={QCMoldMasterId} />
    </React.Fragment>
  );
};
...
export default connect(mapStateToProps, mapDispatchToProps)(QCMasterMold);
