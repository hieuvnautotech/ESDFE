...
import { qcPQCService, bomService } from '@services';
import { addDays, ErrorAlert, SuccessAlert, minusMonths } from '@utils';
import moment from 'moment';
import QCMasterPQCDialog from './QCMasterPQCDialog';
import { PhotoProvider, PhotoView } from 'react-photo-view';
import QCMasterPQCDetailDialog from './QCMasterPQCDetailDialog';

const QCMasterPQC = (props) => {
  const intl = useIntl();
  const date = new Date();
  ...
  const [QCPQCMasterId, setQCPQCMasterId] = useState(null);
  ...
  const columns = [
    {
      ...
      align: 'center',
      filterable: false,
      renderCell: (index) => index.api.getRowIndex(index.row.QCPQCMasterId) + 1 + (state.page - 1) * state.pageSize,
    },
    { field: 'QCPQCMasterId', hide: true },
    ...
    {
      field: 'Detail',
      headerName: intl.formatMessage({ id: 'qcPQC.Detail' }),
      width: 100,
      align: 'center',
      renderCell: (params) => {
        return (
          ...
          >
            {intl.formatMessage({ id: 'qcPQC.Detail' })}
          </Button>
        );
      },
    },
    {
      field: 'Confirm',
      headerName: intl.formatMessage({ id: 'qcPQC.Confirm' }),
      width: 120,
      align: 'center',
      renderCell: (params) => {
        return (
          <Button
            ...
            sx={{ paddingTop: '1px', paddingBottom: '1px' }}
          >
            {intl.formatMessage({ id: params.row.IsConfirm ? 'qcPQC.Confirmed' : 'qcPQC.Confirm' })}
          </Button>
        );
      },
    },
    {
      field: 'QCPQCMasterName',
      headerName: intl.formatMessage({ id: 'qcPQC.QCPQCMasterName' }),
      width: 150,
    },
    {
      field: 'REVDate',
      headerName: intl.formatMessage({ id: 'qcPQC.REVDate' }),
      width: 150,
      valueFormatter: (params) => (params?.value ? moment(params?.value).format('YYYY-MM-DD') : null),
    },
    {
      field: 'ProductList',
      headerName: intl.formatMessage({ id: 'qcPQC.ProductId' }),
      width: 200,
      renderCell: (params) => {
        return (
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <Tooltip title={params.row.ProcessList ?? ''} className="col-text-elip">
            <Typography sx={{ fontSize: 14 }}>{params.row.ProcessList}</Typography>
          </Tooltip>
        );
      },
    },
    {
      field: 'Explain',
      headerName: intl.formatMessage({ id: 'qcPQC.Explain' }),
      width: 150,
    },
    {
      field: 'ImageFile',
      headerName: intl.formatMessage({ id: 'qcPQC.ImageFile' }),
      width: 120,
      align: 'center',
      renderCell: (params) => {
        return (
          params.row.ImageFile != null && (
            <PhotoProvider>
              <PhotoView src={`${BASE_URL}/Image/QCPQC/${params.row.ImageFile}`}>
                <img src={`${BASE_URL}/Image/QCPQC/${params.row.ImageFile}`} style={{ height: 50 }} />
              </PhotoView>
            </PhotoProvider>
          )
        );
    ...
    {
      field: 'Description',
      headerName: intl.formatMessage({ id: 'qcPQC.Description' }),
      width: 200,
      renderCell: (params) => {
        return (
  ...
  useEffect(() => {
    if (!_.isEmpty(updateData) && !_.isEqual(updateData, rowData)) {
      ...
      const index = _.findIndex(newArr, function (o) {
        return o.QCPQCMasterId == updateData.QCPQCMasterId;
      });
      if (index !== -1) {
  ...
  const handleConfirm = async (item) => {
    if (
      ...
      try {
        let res = await qcPQCService.confirmPQCMaster(item);
  ...
  const handleDelete = async (item) => {
    if (
      ...
      try {
        let res = await qcPQCService.deletePQCMaster(item);
        if (res && res.HttpResponseCode === 200) {
          SuccessAlert(intl.formatMessage({ id: 'general.success' }));
          await fetchData();
        } else {
  ...
  const handleCopy = async (row) => {
    if (window.confirm(intl.formatMessage({ id: 'general.confirm_save' }))) {
      try {
        const res = await qcPQCService.copyPQCMaster(row);
  ...
  async function fetchData() {
    ...
    const params = {
      QCPQCMasterName: state.searchData.keyWord,
    ...
    const res = await qcPQCService.getPQCMasterList(params);
  ...
  return (
    <React.Fragment>
      ...
      <MuiDataGrid
        ...
        getRowId={(rows) => rows.QCPQCMasterId}
      ...
      <QCMasterPQCDialog
        ...
        mode={mode}
      />
      <QCMasterPQCDetailDialog isOpen={isShowing2} onClose={toggle2} QCPQCMasterId={QCPQCMasterId} />
      {/* <CopyDialog isOpen={isShowing3} onClose={toggle3} QCPQCMasterId={QCPQCMasterId} /> */}
    </React.Fragment>
  );
};

// const CopyDialog = ({ isOpen, onClose, setNewData, setUpdateData, PackingLabelId, handleUpdateQty }) => {
//   const intl = useIntl();
...
//               onKeyDown={keyPress}
//             />
//             <MuiButton text="copy" color="success" onClick={scanBtnClick} sx={{ ml: 2, whiteSpace: 'nowrap' }} />
//           </Box>
//         </Grid>
//       </Grid>
//     </MuiDialog>
...
export default connect(mapStateToProps, mapDispatchToProps)(QCMasterPQC);
