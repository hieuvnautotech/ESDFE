...
const QCIQCDetailMaterial = ({ QCIQCMasterId, isOpen, dialogState, setDialogState }) => {
  const intl = useIntl();
  ...
  const handleDelete = async (item) => {
    if (
      ...
      try {
        let res = await qcIQCService.deleteIQCM(item);
  ...
  const handleClear = async () => {
    if (window.confirm(intl.formatMessage({ id: 'general.confirm_delete' }))) {
      try {
        let res = await qcIQCService.clearIQCM(QCIQCMasterId);
        if (res && res.HttpResponseCode === 200) {
          SuccessAlert(intl.formatMessage({ id: 'general.success' }));
          await fetchData();
        } else {
  ...
  async function fetchData() {
    ...
    const res = await qcIQCService.getIQCDetailM(params);
    if (res && isRendered) {
      setState({
        ...state,
  ...
  const onSubmit = async (data) => {
    ...
    const res = await qcIQCService.createDetailM(data);
                ...
                state.dataMaterial.map((item, index) => {
                  ...
                  return (
                    <tr key={index}>
                      ...
                      <td style={{ ...style.td, textAlign: 'center' }} rowSpan={state.dataMaterial.Samples}>
...
export default QCIQCDetailMaterial;
