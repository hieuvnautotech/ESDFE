...
import { HoldMaterialService } from '@services';
...
const HoldMaterialDialog = ({ isOpen, onClose, initModal, resetList, unHold, DataSelect }) => {
  ...
  const defaultValue = useMemo(() => {
    return {
      MaterialLotId: initModal?.MaterialLotId,
      Reason: '',
      LotStatus: initModal?.LotStatus,
      IsPicture: false,
      File: '',
    };
  }, [initModal?.MaterialLotId]);
  ...
  const onSubmit = async (data) => {
    ...
    const res = unHold ? await HoldMaterialService.unHold(formData) : await HoldMaterialService.hold(formData);
...
export default HoldMaterialDialog;
