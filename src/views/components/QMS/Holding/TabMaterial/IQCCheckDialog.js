...
const IQCCheckDialog = ({ RowCheck, isOpen, onClose, view, reCheck }) => {
  const intl = useIntl();
  ...
  const onSubmit = async (data) => {
    ...
    if (status == -1) {
      ...
      const res = await HoldMaterialService.reCheck({
        ...data,
  ...
  const handleSetAllResult = (value) => {
    if (value == 'OK') {
      ...
      state.data.forEach((item) => {
        CheckTemplate.push({ ...item, TextValue: '0' });
      });
    ...
    >
      <form onSubmit={handleSubmit}>
        <Grid container rowSpacing={2.5} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
          ...
          <Grid item xs={12}>
            <IQCCheckFormMaterial RowCheck={RowCheck} state={state} setState={setState} view={view} reCheck={reCheck} />
          </Grid>
          {view != true && (
            <Grid item xs={12}>
