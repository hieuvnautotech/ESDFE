...
const HoldSemiMMSActionDialog = ({ isOpen, onClose, initModal, resetList }) => {
  const intl = useIntl();
  ...
  const handleScrap = async () => {
    if (window.confirm(intl.formatMessage({ id: 'Holding.confirm_scrap' }))) {
      try {
        const res = await HoldSemiLotService.scrapMMS({ WOSemiLotMMSId: initModal?.WOSemiLotMMSId });
...
export default HoldSemiMMSActionDialog;
