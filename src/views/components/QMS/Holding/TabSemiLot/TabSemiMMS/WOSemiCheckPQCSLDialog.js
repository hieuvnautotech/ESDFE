...
import { WOService, HoldSemiLotService } from '@services';
...
const WOSemiCheckPQCSLDialog = ({ RowCheck, isOpen, onClose, view, reCheck }) => {
  const intl = useIntl();
  ...
  async function fetchData() {
    const res = await WOService.getPQCDetailSL(RowCheck?.QCPQCMasterId, RowCheck?.WOSemiLotMMSId);
    const value =
      reCheck == true
        ? await WOService.getPQCDetailSLValue(RowCheck?.WOSemiLotMMSId)
        : await HoldSemiLotService.getPQCDetailSLValue(RowCheck?.WOSemiLotMMSId);
  ...
  const formik = useFormik({
    ...
    initialValues:
      view == true
        ...
        : { ...RowCheck, StaffId: null, CheckResult: null, CheckResultName: '' },
  ...
  }, [isOpen, RowCheck?.QCPQCMasterId, RowCheck?.WOSemiLotMMSId]);
  ...
  const onSubmit = async (data) => {
    ...
    if (status > -1) {
      ...
      const res = await HoldSemiLotService.checkPQCSL({
        ...data,
        ValueCheck: table,
        ...
        <Grid container rowSpacing={2.5} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
          ...
          <Grid item xs={12}>
            <Grid container direction="row-reverse">
              {!view && <MuiSubmitButton text="save" loading={state.isSubmit} />}
            </Grid>
          </Grid>
        </Grid>
      </form>
