...
const HoldSemiMMSDialog = ({ isOpen, onClose, initModal, resetList, unHold, DataSelect }) => {
  ...
  const defaultValue = useMemo(() => {
    return {
      WOSemiLotMMSId: initModal?.WOSemiLotMMSId,
      Reason: '',
      LotStatus: initModal?.LotStatus,
      IsPicture: false,
      File: '',
    };
  }, [initModal?.WOSemiLotMMSId]);
  ...
  const onSubmit = async (data) => {
    ...
    //formData.append('row_version', initModal?.row_version);
    const res = unHold ? await HoldSemiLotService.unHoldMMS(formData) : await HoldSemiLotService.holdMMS(formData);
...
export default HoldSemiMMSDialog;
