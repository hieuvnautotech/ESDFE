...
import { HoldFinishGoodService, BuyerQRService } from '@services';
...
import { ErrorAlert, SuccessAlert, PrintBuyer } from '@utils';

const HoldFinishGood = (props) => {
  const intl = useIntl();
  ...
  const columns = [
    {
      ...
      align: 'center',
      filterable: false,
      renderCell: (index) => index.api.getRowIndex(index.row.FGInventoryId) + 1 + (state.page - 1) * state.pageSize,
    },
    { field: 'FGInventoryId', hide: true },
    ...
    { field: 'BoxQR', headerName: intl.formatMessage({ id: 'BoxQR.BoxQR' }), width: 300 },
    {
      field: 'SemiLotCode',
  ...
  const handleHold = async (row) => {
    setMaterialLot(row);
    if (state.searchData.showDelete) toggle();
    else toggle2();
  };
  ...
  const handleScrap = async () => {
    if (window.confirm(intl.formatMessage({ id: 'Holding.confirm_scrap' }))) {
      try {
        const res = await HoldFinishGoodService.scrap({ ListId: DataSelect });
  ...
  const handlePrint = async () => {
    const res = await HoldFinishGoodService.getPrint(DataSelect);
    PrintBuyer(res.Data);
  };
  ...
  async function fetchData() {
    ...
    const res = await HoldFinishGoodService.getAllList(params);

    if (res && isRendered) setState({ ...state, data: res.Data ?? [], totalRow: res.TotalRow, isLoading: false });
  }
  ...
  return (
    <React.Fragment>
      <Grid container direction="row" justifyContent="space-between" alignItems="width-end">
        ...
        <Grid item>
          <FormControlLabel
            ...
            label={intl.formatMessage({ id: state.searchData.showDelete ? 'Holding.UnHoldList' : 'Holding.HoldList' })}
          />
        </Grid>
      </Grid>
      ...
      <MuiDataGrid
        ...
        getRowId={(rows) => rows.FGInventoryId}
        initialState={{
          pinnedColumns: { left: ['id', GRID_CHECKBOX_SELECTION_COL_DEF.field, 'Holding', 'IQCResult'] },
        }}
