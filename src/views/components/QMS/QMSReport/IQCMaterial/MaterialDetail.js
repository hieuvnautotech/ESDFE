...
const MaterialDetail = ({ QCStandard }) => {
  ...
  async function fetchData() {
    ...
    const res = await QCReportService.getMaterialDetail(params);
              ...
              return (
                <Typography sx={{ fontSize: 14 }}>
                  {params.row.stt == 999999
                    ? params.row[key] + '%'
                    : params.row[key] > 0 && params.row.NGQty > 0
                    ? Number(((params.row[key] * 100.0) / params.row.NGQty).toFixed(2)) + '%'
                    : ''}
                </Typography>
              );
            },
  ...
  const handleDownload = async () => {
    try {
      ...
      await QCReportService.downloadMaterialDetail(params);
    } catch (error) {
