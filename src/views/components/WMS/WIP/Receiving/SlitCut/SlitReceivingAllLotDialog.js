...
const SlitReceivingAllLotDialog = ({ isOpen, onClose, SlitSOModel }) => {
  const intl = useIntl();
  ...
  //useEffect
  useEffect(() => {
    if (isOpen && SlitSOModel?.SlitSOId != null) fetchData();
    return () => {
      isRendered = false;
    };
  }, [isOpen, SlitSOModel]);
  ...
  async function fetchData() {
    ...
    const res = await SlitSOService.getSlitSODetailLotBySlitSOId({ SlitSOId: SlitSOModel.SlitSOId });
      ...
      <Grid container flexDirection="row" alignItems="center" justifyContent="center" textAlign="center">
        ...
        <Grid item xs={5.5} md={5.5} style={style.titleMain}>
          <Typography sx={{ fontSize: '25px', fontWeight: 700 }}>
            {intl.formatMessage({ id: 'SlitSO.SlitSOName' })}: {SlitSOModel?.SlitSOName}
          </Typography>
        </Grid>
        <Grid item xs={3.5} md={3.5} sx={{ border: '1px solid #4e4e4e8f', height: '60px', borderLeft: 'none' }}>
          <Box sx={{ borderBottom: '1px solid #4e4e4e8f' }}>
            <Typography sx={{ fontSize: '18px' }}>
              {intl.formatMessage({ id: 'general.createdDate' })}:{' '}
              {moment(SlitSOModel?.createdDate).format('YYYY-MM-DD')}
            </Typography>
          </Box>
          <Box>
            <Typography sx={{ fontSize: '18px' }}>
              {intl.formatMessage({ id: 'general.createdName' })}: {SlitSOModel?.createdName}
            </Typography>
          </Box>
        </Grid>
        <Grid
          className="d-flex justify-content-center align-items-center w-100"
          style={{ border: '1px solid rgba(78, 78, 78, 0.56)', borderTop: 'none' }}
        >
          <Grid
            sx={{
              width: '30%',
          ...
          >
            <Typography sx={{ fontSize: '20px', fontWeight: 600 }}> {SlitSOModel?.ProductCode}</Typography>
          </Grid>
          <Grid
            sx={{
              width: '30%',
              borderRight: '1px solid rgba(78, 78, 78, 0.56)',
              borderLeft: '1px solid rgba(78, 78, 78, 0.56)',
            }}
          >
                ...
                {state.data.map((item, index) => {
                  return (
                    <TableRow key={`MATERIALDetail_${index}`}>
                      ...
                      <TableCell style={style.dataCell}>{item?.MMSLocationCode}</TableCell>
