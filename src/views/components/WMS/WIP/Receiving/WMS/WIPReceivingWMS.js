...
import WIPReceivingWMSDetail from './WIPReceivingWMSDetail';
import WIPReceivingAllLotDialog from './WIPReceivingAllLotDialog';

export default function WIPReceivingWMS() {
  const intl = useIntl();
  ...
  async function fetchData() {
    ...
    const params = {
      ...
      MSOStatus: state.searchData.MSOStatus != '' ? (state.searchData.MSOStatus == '0' ? false : true) : null,
    ...
    const res = await wipReceivingService.getMaterialWMS(params);
  ...
  return (
    <React.Fragment>
      <Grid container direction="row" justifyContent="space-between" alignItems="width-end" sx={{ mb: 1 }}>
        ...
        <Grid item xs>
          <Grid container columnSpacing={2} direction="row" justifyContent="flex-end" alignItems="flex-end">
            ...
            <Grid item style={{ width: '21%' }}>
              <MuiAutocomplete
                label={intl.formatMessage({ id: 'materialSO.MSOStatus' })}
                fetchDataFunc={() => {
                  return {
                    Data: [
                ...
                displayLabel="label"
                displayValue="value"
                onChange={(e, item) => handleSearch(item ? item?.value ?? '' : '', 'MSOStatus')}
                variant="standard"
                fullWidth
              />
            </Grid>
      ...
      <WIPReceivingWMSDetail MSOId={MSOId} fetchDataMaster={fetchData} />

      <WIPReceivingAllLotDialog MSOModel={View} isOpen={isShowing} onClose={toggle} />
    </React.Fragment>
  );
}
