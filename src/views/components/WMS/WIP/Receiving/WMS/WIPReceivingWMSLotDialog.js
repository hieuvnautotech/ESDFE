...
const WIPReceivingWMSLotDialog = ({ isOpen, onClose, fetchDataDetail, MSODetailId }) => {
  const intl = useIntl();
        ...
        return (
          <Grid container spacing={1} alignItems="center" justifyContent="center">
            <IconButton
              ...
              disabled={
                (params.row?.AreaCode != 'MATERIAL' && params.row?.AreaCode != 'WIP') ||
  ...
  async function fetchData() {
    ...
    const res = await wipReceivingService.getDetailLotWMS(params);
  ...
  const scanBtnClick = async () => {
    ...
    //   MaterialLotCode: lot,
    // });
    const res = await wipReceivingService.scanWIPDetailLotWMS({ MSODetailId: MSODetailId, MaterialLotCode: lot });
  ...
  const handleDelete = async (row) => {
    ...
    if (window.confirm(message)) {
      try {
        let res = await wipReceivingService.deleteScanWMS({
          MSODetailId: row.MSODetailId,
...
export default WIPReceivingWMSLotDialog;
