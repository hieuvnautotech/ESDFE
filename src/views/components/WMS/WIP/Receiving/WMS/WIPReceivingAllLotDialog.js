...
const SlitReceivingAllLotDialog = ({ isOpen, onClose, MSOModel }) => {
  ...
  async function fetchData() {
    ...
    const res = await wipReceivingService.getMSODetailLotByMSOId({ MSOId: MSOModel.MSOId });
      ...
      <Grid container flexDirection="row" alignItems="center" justifyContent="center" textAlign="center">
        ...
        <Grid item xs={5.5} md={5.5} style={style.titleMain}>
          <Typography sx={{ fontSize: '25px', fontWeight: 700 }}>
            {intl.formatMessage({ id: 'SlitSO.SlitSOName' })}: {MSOModel?.MSOName}
          </Typography>
        </Grid>
        ...
        >
          ...
          >
            <Typography sx={{ fontSize: '15px' }}> {MSOModel?.ProductList}</Typography>
          </Grid>
          <Grid
            sx={{
              width: '30%',
