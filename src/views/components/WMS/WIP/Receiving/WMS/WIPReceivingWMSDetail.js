...
import WIPReceivingWMSLotDialog from './WIPReceivingWMSLotDialog';

export default function WIPReceivingWMSDetail({ MSOId }) {
  const intl = useIntl();
  ...
  async function fetchData(Id) {
    ...
    const res = await wipReceivingService.getDetailWMS(params);
...
const DetailPanelContent = ({ row: rowProp, intl, isShowing, handleScan }) => {
  ...
  const fetchDetailData = async () => {
    if (isDetailRendered) {
      ...
      const res = await wipReceivingService.getDetailHistoryWMS(params);
