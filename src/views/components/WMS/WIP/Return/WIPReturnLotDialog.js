...
const WIPReturnLotDialog = ({ isOpen, onClose, RMModel }) => {
  const intl = useIntl();
  ...
  //useEffect
  useEffect(() => {
    if (isOpen && RMModel?.WIPRMId != null) fetchData();
    return () => {
      isRendered = false;
    };
  }, [isOpen, RMModel]);
  ...
  async function fetchData() {
    ...
    const res = await WIPReturnService.getViewLot({
      WIPRMId: RMModel.WIPRMId,
      page: 0,
      pageSize: 0,
      isActived: true,
    });
...
export default WIPReturnLotDialog;
