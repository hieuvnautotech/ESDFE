...
import { bomService, MaterialSOService } from '@services';
import { ErrorAlert, SuccessAlert, addDays } from '@utils';
import moment from 'moment';
import React, { useEffect, useRef, useState } from 'react';
import { useIntl } from 'react-intl';
import MaterialSODialog from './MaterialSODialog';
import MaterialSODetail from './MaterialSODetail';

export default function MaterialSO() {
  const intl = useIntl();
  const date = new Date();
  ...
  useEffect(() => {
    if (!_.isEmpty(updateData) && !_.isEqual(updateData, rowData)) {
      ...
      const index = _.findIndex(newArr, function (o) {
        return o.MSOId == updateData.MSOId;
      });
      if (index !== -1) {
  ...
  const handleDelete = async (item) => {
    if (
      ...
      try {
        let res = await MaterialSOService.deleteMaterialSO(item);
