...
const FQCReceivingAllLotDialog = ({ isOpen, onClose, RMModel }) => {
  const intl = useIntl();
  ...
  async function fetchData() {
    ...
    const res = await FGReceivingService.getFQCSODetailAll({
            ...
            <Table sx={{ minWidth: 500 }} aria-label="custom pagination table">
              <TableBody>
                <TableRow sx={{ backgroundColor: '#ececec' }}>
                  ...
                  <TableCell style={style.titleCell}>{intl.formatMessage({ id: 'buyer.BuyerCode' })}</TableCell>
                  <TableCell style={style.titleCell}>{intl.formatMessage({ id: 'WO.SemiLotCode' })}</TableCell>
                  <TableCell style={style.titleCell}>{intl.formatMessage({ id: 'location.LocationCode' })}</TableCell>
...
export default FQCReceivingAllLotDialog;
