...
const SlitReturnLotDialog = ({ isOpen, onClose, RMModel }) => {
  const intl = useIntl();
  ...
  //useEffect
  useEffect(() => {
    if (isOpen && RMModel?.RMId != null) fetchData();
    return () => {
      isRendered = false;
    };
  }, [isOpen, RMModel]);
  ...
  async function fetchData() {
    ...
    const res = await MaterialReturnService.getReturnMaterialDetail({
      RMId: RMModel.RMId,
      page: 0,
      pageSize: 0,
      isActived: true,
    ...
    >
      <Grid container flexDirection="row" alignItems="center" justifyContent="center" textAlign="center">
        <Grid item xs={3} md={3} sx={{ ...style.titleMain, borderRight: 'none' }}>
          <Typography sx={{ fontSize: '30px', fontWeight: 800 }}>SAMJU</Typography>
        </Grid>
        <Grid item xs={5.5} md={5.5} style={style.titleMain}>
          <Typography sx={{ fontSize: '25px', fontWeight: 700 }}>
            {intl.formatMessage({ id: 'returnMaterial.RMName' })}: {RMModel?.RMName}
          </Typography>
        </Grid>
        <Grid item xs={3.5} md={3.5} sx={{ border: '1px solid #4e4e4e8f', height: '60px', borderLeft: 'none' }}>
          <Box sx={{ borderBottom: '1px solid #4e4e4e8f' }}>
            <Typography sx={{ fontSize: '18px' }}>
              {intl.formatMessage({ id: 'general.createdDate' })}: {moment(RMModel?.createdDate).format('YYYY-MM-DD')}
            </Typography>
          </Box>
          <Box>
            <Typography sx={{ fontSize: '18px' }}>
...
export default SlitReturnLotDialog;
