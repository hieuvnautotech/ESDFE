...
import { bomService, ReturnMaterialService } from '@services';
import { ErrorAlert, SuccessAlert, addDays } from '@utils';
import moment from 'moment';
import React, { useEffect, useRef, useState } from 'react';
import { useIntl } from 'react-intl';
import SlitReturnDialog from './SlitReturnDialog';
import SlitReturnDetail from './SlitReturnDetail';
import SlitReturnLotDialog from './SlitReturnLotDialog';
import VisibilityIcon from '@mui/icons-material/Visibility';

export default function SlitReturn() {
  const intl = useIntl();
  ...
  const handleDelete = async (item) => {
    if (
      ...
      try {
        let res = await ReturnMaterialService.deleteReturnMaterial(item);
  ...
  async function fetchData() {
    ...
    const res = await ReturnMaterialService.getReturnMaterialList(params);
  ...
  return (
    <React.Fragment>
      ...
      <SlitReturnLotDialog RMModel={View} isOpen={isShowing2} onClose={toggle2} />
    </React.Fragment>
  );
}
