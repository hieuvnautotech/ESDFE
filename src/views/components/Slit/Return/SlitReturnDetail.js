...
export default function SlitReturnDetail({ RMId }) {
  const intl = useIntl();
  ...
  async function fetchData(Id) {
    ...
    const res = await ReturnMaterialService.getReturnMaterialDetail(params);
  ...
  const scanBtnClick = async () => {
    ...
    const res = await ReturnMaterialService.scanReturnMaterialDetailLot({ RMId: RMId, MaterialLotCode: lot });
...
const DetailPanelContent = ({ row: rowProp, intl, fetchData }) => {
  ...
  const fetchDetailData = async () => {
    if (isDetailRendered) {
      ...
      const params = {
        ...
        RMId: detailPanelState.RMId,
        MaterialId: detailPanelState.MaterialId,
      };

      const res = await ReturnMaterialService.getReturnMaterialDetailHistory(params);
  ...
  const handleDelete = async (item) => {
    if (window.confirm(intl.formatMessage({ id: 'general.confirm_delete' }))) {
      try {
        let res = await ReturnMaterialService.deleteReturnMaterialDetail(item);
        if (res && res.HttpResponseCode === 200) {
          SuccessAlert(intl.formatMessage({ id: 'general.success' }));
          await fetchData(rowProp.RMId);
        } else {
          ErrorAlert(intl.formatMessage({ id: res.ResponseMessage }));
        }
      } catch (error) {
