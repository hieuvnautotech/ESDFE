...
import { SlitStockService, IQCReceivingService } from '@services';
...
const MaterialOKTab = (props) => {
  ...
  async function fetchData() {
    ...
    const res = await SlitStockService.getMaterialList(params);
    if (res && isRendered)
      setState({
        ...State,
        data: res.Data ?? [],
  ...
  const columns = [
    ...
    { field: 'MaterialName', headerName: intl.formatMessage({ id: 'material.MaterialName' }), flex: 0.4 },
...
const DetailPanelContent = ({ row: rowProp, intl, searchData, getDataPrint: getDataPrint }) => {
  ...
  const fetchDetailData = async () => {
    ...
    const res = await SlitStockService.getMaterialLotList(params);
