...
import SlitOrderDetailDialog from './SlitOrderDetailDialog';

export default function SlitOrderDetail({ SlitOrderId, modeAction, isActivedRow }) {
  const intl = useIntl();
  ...
  const [SlitOrderDetailId, setSlitOrderDetailId] = useState(null);
  ...
  const columns = [
    {
      ...
      align: 'center',
      filterable: false,
      renderCell: (index) => index.api.getRowIndex(index.row.SlitOrderDetailId) + 1 + (state.page - 1) * state.pageSize,
    },
    { field: 'SlitOrderDetailId', hide: true },
          ...
          <Grid container spacing={1} alignItems="center" justifyContent="center">
            <Grid item xs={4} style={{ textAlign: 'center' }}>
              <IconButton
                ...
                disabled={modeAction || params.row?.CheckDeleteStatus}
              >
    ...
    {
      field: 'OrderQty',
      headerName: intl.formatMessage({ id: 'slitOrder.OrderQty' }),
      width: 150,
    },
    {
      field: 'DoneQty',
      headerName: intl.formatMessage({ id: 'slitOrder.DoneQty' }),
      width: 150,
    },
    {
      field: 'Description',
  ...
  }, [state.page, state.pageSize, state.searchData.showDelete, SlitOrderId]);
  ...
  useEffect(() => {
    if (!_.isEmpty(updateData) && !_.isEqual(updateData, rowData)) {
      ...
      const index = _.findIndex(newArr, function (o) {
        return o.SlitOrderDetailId == updateData.SlitOrderDetailId;
      });
      if (index !== -1) {
  ...
  const handleDelete = async (item) => {
    if (
      ...
      try {
        let res = await SlitOrderService.deleteSlitOrderDetail(item);
  ...
  const handleAddMaterial = async (row) => {
    setSlitOrderDetailId(row.SlitOrderDetailId);
    toggle2();
  };
  ...
  async function fetchData() {
    ...
    const params = {
      ...
      SlitOrderDetailId: state.searchData.SlitOrderDetailId,
      page: state.page,
      pageSize: state.pageSize,
      isActived: state.searchData.showDelete,
    };

    const res = await SlitOrderService.getSlitOrderDetailList(params);
  ...
  return (
    <React.Fragment>
      <Grid container direction="row" justifyContent="space-between" alignItems="width-end">
        <Grid item xs={3}>
          <MuiButton
            ...
            onClick={handleAdd}
            sx={{ mt: 1 }}
            disabled={(SlitOrderId ? false : true) || modeAction}
          />
        </Grid>
        <Grid item xs>
          <Grid container columnSpacing={2} direction="row" justifyContent="flex-end" alignItems="flex-end">
            {/* <Grid item style={{ width: '21%' }}>
              <MuiAutocomplete
                disabled={SlitOrderId ? false : true}
                label={intl.formatMessage({ id: 'slitOrder.ProcessId' })}
                fetchDataFunc={bomService.getProcess}
                displayLabel="commonDetailName"
                displayValue="commonDetailId"
                onChange={(e, item) => handleSearch(item ? item.commonDetailId ?? null : null, 'ProcessId')}
                variant="standard"
                fullWidth
              />
            </Grid> */}
      ...
      <MuiDataGrid
        ...
        getRowId={(rows) => rows.SlitOrderDetailId}
      ...
      {/* <SlitOrderDetailMaterialDialog SlitOrderDetailId={SlitOrderDetailId} isOpen={isShowing2} onClose={toggle2} /> */}
    </React.Fragment>
  );
}
