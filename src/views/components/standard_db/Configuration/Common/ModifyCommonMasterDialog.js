...
const ModifyCommonMasterDialog = (props) => {
  ...
  const schema = yup.object().shape({
    ...
    commonMasterCode: yup.string().required(),
            ...
            <Grid container spacing={2}>
              <Grid item xs={RoleArr.includes('ROOT') ? 9 : 12}>
                <TextField
                  ...
                  {...register('commonMasterCode', {
                    // onChange: (e) => handleInputChange(e)
                  })}
                  //defaultValue={initModal.commonMasterName}
                  error={!!errors?.commonMasterCode}
                  helperText={errors?.commonMasterCode ? errors.commonMasterCode.message : null}
                />
              </Grid>
