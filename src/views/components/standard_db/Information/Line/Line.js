...
import { FormControlLabel, Switch, Typography, Tooltip, Badge } from '@mui/material';
...
import { ErrorAlert, SuccessAlert, PrintLine } from '@utils';
...
const Line = (props) => {
  ...
  const handlePrint = async () => {
    const res = await lineService.PrintLine(DataPrint);
    PrintLine(res.Data);
  };
  ...
  const fetchData = async () => {
    ...
    const params = {
      ...
      Location: lineState.searchData.Location,
  ...
  const columns = [
    ...
    { field: 'LineName', headerName: '', hide: true },
    {
      field: 'id',
      headerName: '',
      width: 40,
    ...
    { field: 'LineName', headerName: intl.formatMessage({ id: 'line.LineName' }), width: 300 },
    {
      ...
      width: 200,
      renderCell: (params) => {
        return <span>{intl.formatMessage({ id: params?.row?.Location })}</span>;
      },
    },
    {
      field: 'Description',
  ...
  return (
    <React.Fragment>
      <Grid container spacing={2} direction="row" justifyContent="space-between" alignItems="flex-end">
        ...
        <Grid item xs>
          <MuiSearchField
            ...
            name="LineName"
            onClick={fetchData}
            onChange={(e) => changeSearchData(e.target.value.trim(), 'LineName')}
          />
        </Grid>
        ...
        <Grid item xs>
          <MuiAutocomplete
            translationLabel
            label={intl.formatMessage({ id: 'line.Location' })}
            fetchDataFunc={lineService.getLocationList}
            displayLabel="commonDetailLanguge"
            displayValue="commonDetailCode"
            onChange={(e, item) => changeSearchData(item ? item.commonDetailCode ?? null : null, 'Location')}
            variant="standard"
          />
        </Grid>
        <Grid item xs sx={{ display: 'flex', justifyContent: 'right', whiteSpace: 'nowrap' }}>
      ...
      <MuiDataGrid
        ...
        initialState={{
          pinnedColumns: { left: ['id', GRID_CHECKBOX_SELECTION_COL_DEF.field, 'LineName'], right: ['action'] },
        }}
        checkboxSelection
      />
