...
import { LineStockService, IQCReceivingService } from '@services';
import { PrintMaterial } from '@utils';
import moment from 'moment';
import React, { useEffect, useRef, useState } from 'react';
import { useIntl } from 'react-intl';

const LineStockOKTab = (props) => {
  const intl = useIntl();
  ...
  async function fetchData() {
    ...
    const res = await LineStockService.getMaterialList(params);
    if (res && isRendered)
      setState({
        ...State,
        data: res.Data ?? [],
...
const DetailPanelContent = ({ row: rowProp, intl, searchData, getDataPrint: getDataPrint }) => {
  ...
  const fetchDetailData = async () => {
    ...
    const res = await LineStockService.getMaterialLotList(params);
