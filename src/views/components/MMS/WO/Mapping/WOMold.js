...
const WOMold = ({ WOProcessId, isOpen, onClose }) => {
  const intl = useIntl();
  ...
  const columns = [
    {
      ...
      align: 'center',
      filterable: false,
      renderCell: (index) => index.api.getRowIndex(index.row.WOProcessMoldId) + 1 + (state.page - 1) * state.pageSize,
    },
    { field: 'WOProcessMoldId', hide: true },
  ...
  useEffect(() => {
    if (!_.isEmpty(updateData) && !_.isEqual(updateData, rowData)) {
      ...
      const index = _.findIndex(newArr, function (o) {
        return o.WOProcessMoldId == updateData.WOProcessMoldId;
      });
      if (index !== -1) {
  ...
  const handleDelete = async (item) => {
    if (
      ...
      try {
        let res = await WOService.deleteProcessMold(item);
  ...
  const onSubmit = async (data) => {
    if (mode == CREATE_ACTION) {
      const res = await WOService.createProcessMold(data);
      ...
      if (res.HttpResponseCode === 300) {
        if (window.confirm(intl.formatMessage({ id: 'WO.error_line_dup_other' }))) {
          try {
            const res = await WOService.createProcessMoldDup(data);
            if (res.HttpResponseCode === 200) {
              SuccessAlert(intl.formatMessage({ id: res.ResponseMessage }));
              setNewData({ ...res.Data });
              return;
      ...
      const res = await WOService.modifyProcessMold({
        ...data,
  ...
  return (
    <MuiDialog
      maxWidth="md"
      title={intl.formatMessage({ id: 'WO.mold' })}
      isOpen={isOpen}
        ...
        <Grid container columnSpacing={{ xs: 1, sm: 2, md: 3 }} sx={{ alignItems: 'center' }}>
          <Grid item xs={mode == UPDATE_ACTION ? 4 : 6}>
            <MuiAutocomplete
              ...
              label={intl.formatMessage({ id: 'WO.mold' })}
              fetchDataFunc={() => WOService.getMold(WOProcessId)}
              displayLabel="MoldName"
              displayValue="MoldId"
              value={
                values.MoldId
                  ? {
              ...
              onChange={(e, value) => {
                setFieldValue('MoldName', value?.MoldName || '');
                setFieldValue('MoldId', value?.MoldId || null);
              }}
